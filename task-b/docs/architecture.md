...
- 标准问答 Agent 的 `agent_loop` 对应本项目的受控调用入口，`tool_registry` 对应工具白名单，`result_projection` 对应证据投影，Claim 校验对应 citation validator。
- 标准撰写 Agent 的 `StateGraph`、`tools_condition` 和依赖注入测试对应未来 S3 的多步流程；本 S2 不复制其复杂会话和编辑能力。
- Java 综合平台的职责边界对应本项目未来可接入的认证网关；当前作业用 `user_group` 模拟已认证身份，不把鉴权写成前端信任。
- Claude Code、Pi 和 DeepSeek Harness 的共同启发是：模型循环、工具权限、事实记录、上下文和验证属于不同职责；本项目只实现题目需要的最小子集。
