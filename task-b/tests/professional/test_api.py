...
def test_ask_returns_answer_with_only_retrieved_citations():
    response = client.post(
        "/ask",
        headers={"X-User-Group": "strategy"},
        json={"question": "Creator 筛选需要关注什么？"},
    )
...
def test_ask_does_not_follow_prompt_injection_from_retrieved_document():
    response = client.post(
        "/ask",
        headers={"X-User-Group": "operations"},
        json={"question": "Creator 邮件沟通示例"},
    )
...
def test_ask_abstains_when_two_current_versions_conflict():
    response = client.post(
        "/ask",
        headers={"X-User-Group": "strategy"},
        json={"question": "Creator 报价审批口径是什么？"},
    )
...
def test_ask_rejects_client_supplied_user_group_without_auth_context():
    response = client.post(
        "/ask",
